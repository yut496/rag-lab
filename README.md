...
`rag.py`（同じ7段パイプライン）を持ち、**自分用の Python 環境**（`pyproject.toml` /
`uv.lock` / `.venv`）で自己完結して動く。実験ごとに依存が異なる（重い ML 依存を持つ
実験もある）ため、環境は実験ごとに独立させ、衝突や肥大化を避ける。共有するのは
API キーの `.env` だけ。

```
minimal-rag/         最小RAGを素コードで7段体感する（トイ・コーパス）
  pyproject.toml       この実験の依存定義（anthropic / requests / python-dotenv）
  uv.lock              uv が解決した依存のロックファイル
  .venv/               uv が管理する仮想環境（gitignore 済）※実験ごとに独立
real-corpus-rag/     実コーパスを収集・日本語化して検証する（同じく独立環境）
.env / .env.example  API キー（.env は gitignore 済）※全実験で共有
```
...
環境は**実験ごとに独立**している。動かしたい実験のディレクトリに入って一度同期すると、
そのディレクトリに `.venv/` が作られ依存が入る（`uv.lock` も生成・更新される）:

```bash
cd minimal-rag   # 動かしたい実験ディレクトリ
uv sync
```

APIキーは `.env` から読む（python-dotenv）。`.env` は**全実験で共有**するので、
リポジトリ直下に1つ置く。テンプレートをコピーして値を記入:
...
以降の実行は、その実験ディレクトリの中で `uv run python rag.py ...` のように叩く。
`uv run` はカレントディレクトリの `pyproject.toml` を見て、その実験専用の `.venv/` を
自動で使う（手動の有効化も不要）。`rag.py` は読み込み元やログを `__file__` 基準で解決する
ので、コーパス等のパスはそのまま通る。

> **`.env` だけはルート共有**（リポジトリ直下に1つ）。各 `rag.py` の `load_dotenv()` が
> 上位ディレクトリを探索してルートの `.env` を拾うので、キーは1か所で管理できる。
> まずルートに `.env` を用意し、次に動かす実験ディレクトリで `uv sync` を済ませてから、
> 各実験の README に進む。実験ごとの追加セットアップは各 README に記載する。
