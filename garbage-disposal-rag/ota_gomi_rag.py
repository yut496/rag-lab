...
def llm_smoke():
    """call_llm の疎通確認。"test" を渡し、非空のテキスト応答を表示して返す。

    キーは export せず、リポジトリの .env から読む（python-dotenv）。
    """
    from dotenv import load_dotenv
    load_dotenv()   # cwd〜親をたどり rag-lab/.env を読む（os.environ に注入）
    out = call_llm("test")
    print("[llm_smoke] model:", LLM_MODEL)
    print("[llm_smoke] response:", repr(out))
    assert isinstance(out, str) and out.strip(), "空応答（call_llm が応答テキストを返していない）"
    return out
